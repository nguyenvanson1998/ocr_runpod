...
@app.post(path="/process_document")
async def process_document_file(
    ...
    try:
        ...
        logger.info(f"Processing completed for {batch_unique_dir}")
        ...
        for file_idx, pdf_name in enumerate(pdf_file_names):
            ...
            for content_item in current_file_content_list:
                ...
                if item_type in ["text", "equation"]:
                    ...
                    if text_content.strip():  
                        ...
                        current_text_buffer_pages.add(item_page_idx)

                    while len(current_text_buffer) >= CHUNK_SIZE:
                        chunk = current_text_buffer[:CHUNK_SIZE].strip()
                        if chunk:  
                            processed_results.append({
                                "content": chunk,
                                "metadata": {
                                    "pageStart": min(current_text_buffer_pages) if current_text_buffer_pages else 0,
                                    "pageEnd": max(current_text_buffer_pages) if current_text_buffer_pages else 0,
                                },
                                "image_description": None,
                                "image_base64": None
                            })
                       
                        current_text_buffer = current_text_buffer[CHUNK_SIZE:].lstrip()
                        if not current_text_buffer.strip():
                            current_text_buffer_pages = set()
                            break
                    ...
                    table_text = "\n".join(parts)
                    if table_text.strip():
                        current_text_buffer += table_text + "\n"
                        current_text_buffer_pages.add(item_page_idx)
            ...
            if current_text_buffer.strip():
                final_chunk_content = current_text_buffer.strip()
                final_chunk_pages_start = min(current_text_buffer_pages) if current_text_buffer_pages else 0
                final_chunk_pages_end = max(current_text_buffer_pages) if current_text_buffer_pages else 0

                if (processed_results and
                    processed_results[-1].get("image_description") is None and
                    len(final_chunk_content) < MIN_CHUNK_SIZE):
                   
                    last_text_chunk = processed_results[-1]
                    last_text_chunk["content"] += "\n" + final_chunk_content
                    last_text_chunk["metadata"]["pageEnd"] = max(
                        last_text_chunk["metadata"]["pageEnd"],
                        final_chunk_pages_end
                    )
                    logger.info(f"Merged small chunk (length {len(final_chunk_content)}) with previous text chunk.")
                else:
                    processed_results.append({
                        "content": final_chunk_content,
                        "metadata": {
                            "pageStart": final_chunk_pages_start,
                            "pageEnd": final_chunk_pages_end,
                        },
                        "image_description": None,
                        "image_base64": None
                    })
                    logger.info(f"Added remaining text as a new chunk (length {len(final_chunk_content)}).")

        logger.info(f"All files processed and post-processed. Total chunks: {len(processed_results)}")
